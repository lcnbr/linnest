...
#let _selected-label(
  ...
  if show-edge-index {
    pieces.push([$#edge-index-prefix _ #edge-index(edge, fields: edge-index-fields)$])
  }
  if show-half-edge-index {
    pieces.push(_prefixed-content(half-edge-index-prefix, dangling-half-edge-index(edge)))
  }
  if show-particle {
    let name = particle-name(edge)

    let label = if map.at(name,default:none) != none{
      map.at(name).label
    } else{
      [#name]
    }
    pieces.push(_prefixed-content(particle-prefix, label))
  }
